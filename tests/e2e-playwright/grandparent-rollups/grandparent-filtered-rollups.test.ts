...
test.describe('Grandparent Rollup Filtered Operations Testing', () => {
  test('Should perform filtered COUNT across grandparent relationships with complex WHERE clause', async ({ page }) => {
    ...
    // 4b. Fill Grandparent Relationship Field Path (Contact.Account.NumberOfEmployees or Contact.Account.AnnualRevenue)
    ...
    if (await grandparentPathInput.isVisible()) {
      ...
      const targetField = await page.locator('input[name="RollupFieldOnLookupObject__c"]').inputValue();
      await grandparentPathInput.fill(`Contact.Account.${targetField}`);
      console.log(`✅ Filled Grandparent Relationship Field Path: Contact.Account.${targetField}`);
    }
