...
test.describe('Grandparent Rollup Filtered Operations Testing', () => {
  test('Should perform filtered COUNT across grandparent relationships with complex WHERE clause', async ({ page }) => {
    ...
    if (await startButton.isVisible()) {
      ...
      await sfHelper.takeScreenshot('filtered-count-rollup-started');
    }
    ...
    await sfHelper.takeScreenshot('filtered-count-rollup-completed');
    ...
    try {
      ...
      const expectedResult = 2; // We created 2 cases matching the filter: Priority = 'High' AND Status != 'Closed'
      ...
      if (rollupSuccessful) {
        ...
        console.log('   - Successfully executed and validated filtered grandparent rollup');
